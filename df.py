...
import plotly
import json
df=pd.read_pickle("zomato.csv" + '.pkl',compression='zip')
fdf=df.drop('url',axis=1)
#wrdf code here

fdf.rate=fdf.rate.fillna("-1/5")   
for i in range(len(fdf.rate)):
    fdf.rate[i]=fdf.rate[i].split('/')[0]
for i in range(len(fdf.rate)):
    if fdf.rate[i]=='NEW' or  fdf.rate[i]=='-':
        fdf.rate[i]='-1'       
fdf.rate=fdf.rate.astype(float)   
fdf.dish_liked=fdf.dish_liked.fillna('not mentioned')
fdf['approx_cost(for two people)']=fdf['approx_cost(for two people)'].astype(str)
 

sum2=0
men=0
g=1
for i in range(len(fdf['approx_cost(for two people)'])):
    for j in fdf['approx_cost(for two people)'][i]:
        if j==',':
            fdf['approx_cost(for two people)'][i]=fdf['approx_cost(for two people)'][i].replace(',','')
            
            
fdf['approx_cost(for two people)'].unique()

ts=0
tsc=0
for i in fdf['approx_cost(for two people)']:
    if i !='nan':
        ts=ts+int(i)
        tsc=tsc+1
...
fdf['approx_cost(for two people)']=fdf['approx_cost(for two people)'].fillna(mean)
fdf['approx_cost(for two people)']=fdf['approx_cost(for two people)'].astype('float')
fdf.phone=fdf.phone.fillna('not available') 
fdf.location=fdf.location.fillna('not available')
fdf.rest_type=fdf.rest_type.fillna('not available')
fdf.cuisines=fdf.cuisines.fillna('not available')

wrdf=fdf.drop('reviews_list',axis=1)
wrdf=wrdf.drop_duplicates()
def getlocationdrop():
    j=0
    for i in wrdf['location']:
        i=str(i)
        wrdf.location[j]=i.replace(" ","_")
        j=j+1
    locations=wrdf.location.unique().tolist()
    return locations
def getrestypedrop():
    j=0
    for i in wrdf['rest_type']:
        i=str(i)
        wrdf.rest_type[j]=i.replace(" ","_")
        j=j+1
...
def getcostlist():
    fdf['approx_cost(for two people)']=fdf['approx_cost(for two people)'].fillna(mean)
    fdf['approx_cost(for two people)']=fdf['approx_cost(for two people)'].astype('float')
    wrdf['approx_cost(for two people)']=wrdf['approx_cost(for two people)'].fillna(mean)
